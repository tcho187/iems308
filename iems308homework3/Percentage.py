...
# RE part
#Can be number or letter e.g. one or . or "" to start
import re
def preprocessor(text):
    text = re.search(r'(\"*\-*(?:\d*\.?\d*|\w+\-*\w+)\s*)(?:\%|percent|percent points)\"*',text)
    if not text:
        text=""
    else:
        text= text.group(0)
...
percentage=df3['label'].drop_duplicates().to_csv('./percentage.csv')
